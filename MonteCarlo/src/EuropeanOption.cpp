...
{
    return (log(S0/K)+
       (r+0.5*pow(sigma,2.0))*get_t())
       /(sigma*sqrt(get_t()));
}

double EurCall::d_minus(double S0, double sigma, double r)
{
    return d_plus(S0,sigma,r)-sigma*sqrt(get_t());
}
   ...
   double sigma, double r)
{
    return S0*N(d_plus(S0,sigma,r))
       -K*exp(-r*get_t())*N(d_minus(S0,sigma,r));
}
   ...
   double sigma, double r)
{
    ...
    return S0*exp(-d_plus(S0,sigma,r)*d_plus(S0,sigma,r)/2)*sqrt(get_t())
       /sqrt(2.0*pi);
}
